...
public class CommonUtils {
    ...
    public static Intent getInstallIntent(File apkFile) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            String auth = ControlLibrary.get().getResources().getString(R.string.AutoUpdateprovide);
            uri = FileProvider.getUriForFile(ControlLibrary.get(), auth, apkFile);
...
//        return AndroidProcesses.isMyProcessInTheForeground();
    ...
    public static Class<Activity> getLaunchActivity() {
        try {
            PackageInfo packageInfo = ControlLibrary.get().getPackageManager().getPackageInfo(ControlLibrary.get().getPackageName(), 0);
            ...
            List<ResolveInfo> list = ControlLibrary.get().getPackageManager().queryIntentActivities(resovleIntent, 0);
